CREATE TABLE `com_code_info` (
                                 `code_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '코드ID|코드ID|',
                                 `code_name` varchar(50) NOT NULL COMMENT '코드명|코드명|',
                                 `description` varchar(200) DEFAULT NULL COMMENT '설명|코드에 대한 설명|',
                                 ...
                                 PRIMARY KEY (`code_id`),
                                 UNIQUE KEY `com_code_info_ux01` (`code_name`)
...
CREATE TABLE `com_code_group` (
                                  ...
                                  `upper_code_group_id` varchar(4) DEFAULT NULL COMMENT '상위코드그룹ID|상위코드그룹ID|',
                                  `code_group_name` varchar(50) NOT NULL COMMENT '코드그룹명|코드그룹명|',
                                  `code_group_description` varchar(200) DEFAULT NULL COMMENT '코드그룹설명|코드그룹설명|',
                                  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일시|생성일시|',
                                  ...
                                  UNIQUE KEY `com_code_group_ux01` (`code_group_name`),
                                  KEY `com_code_group_fk01` (`upper_code_group_id`),
                                  CONSTRAINT `com_code_group_fk01` FOREIGN KEY (`upper_code_group_id`) REFERENCES `com_code_group` (`code_group_id`)
...
CREATE TABLE `com_code_group_history` (
                                          ...
                                          `bfchg_code_group_name` varchar(50) NOT NULL COMMENT '변경전코드그룹명|변경전 코드그룹명|',
                                          `aftch_code_group_name` varchar(50) DEFAULT NULL COMMENT '변경후코드그룹명|변경후그룹코드명|',
                                          `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일시|생성일시|',
                                          `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시|수정일시|',
...
CREATE TABLE `com_code` (
                            `code_group_id` varchar(4) NOT NULL COMMENT '코드그룹ID|com_code_group.id|',
                            `code_id` int(11) NOT NULL COMMENT '코드ID|com_code_info.id|',
